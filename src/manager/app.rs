...
pub trait ApplicationApi: Send + Sync {
    ...
    ///
    /// Unlike [`Self::window_list`] this reads only the ID off each element (no
    /// title, role, or rule matching), making it cheap enough for the destroy
    /// path's liveness check.
